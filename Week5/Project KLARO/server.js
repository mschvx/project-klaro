...
const server = http.createServer((req, res) => {
    ...
    if (parsedUrl.pathname === '/save' && req.method === 'POST') {
        ...
        req.on('end', () => {
            ...
            try {
                // Try to parse the incoming body and write a formatted JSON file where
                // each project's data array is compact (inline) but the file remains
                // pretty-printed for readability — matches the user's requested style.
                let outBody = body;
                try {
                    const parsed = JSON.parse(body);
                    if (parsed && typeof parsed === 'object' && Array.isArray(parsed.projects)) {
                        const projStr = parsed.projects.map(p => {
                        ...
                        // Fallback to a standard pretty-printed JSON if structure differs
                        outBody = JSON.stringify(parsed, null, 2);
                    }
                } catch (e) {
                    // If parsing fails, write the raw body (preserve previous behavior)
                    outBody = body;
                }

                fs.writeFileSync(jsonPath, outBody);
            } catch (writeErr) {
            ...
            execFile(rscriptPath, ['R/Minimization.R'], { cwd: __dirname, windowsHide: true }, (err, stdout, stderr) => {
                if (err) {
                    ...
                    try {
                        ...
                        // Attach the R error message so the UI can show it instead of a stale numeric result
                        try {
                            toSend.error = String(stderr || (err && err.message) || 'Rscript error');
                        } catch (e) { /* ignore */ }
